 ...
 * 좁은 통로에서는 거리가 계속 변하는 것 자체가 어색해서 결국 "거리를 줄이는" 방식을 버렸다.
 * 지금은 고정 오프셋에서 벽 밖으로 밀어내기만 하고, 가리는 벽은 반투명하게 만든다.
 * 밀어내기는 옆으로 움직이는 거라 거리가 조금 변하긴 하지만 아주 매끄럽다.
 * 그래서 이 스크립트가 볼 것은 "값이 몇 종인가" 가 아니라 "프레임 사이에 튀는가" 다.
 */
...
console.log(
  maxJump < 0.1
    ? '  → 프레임 사이 변화가 매끄럽다.'
    : '  → 프레임 사이에 튄다. 거리를 줄이는 로직이 다시 들어갔는지 확인할 것.',
)
